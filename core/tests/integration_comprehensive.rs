...
fn test_full_routing_pipeline() {
    ...
    route1.examples = vec!["Where's my invoice?".into(), "I need a receipt".into()];
    ...
    route2.examples = vec!["App is crashing".into(), "Can't login".into()];
    ...
    route3.examples = vec!["What's the price?".into(), "I want to upgrade".into()];
...
fn test_routing_keyword_matching() {
    ...
    assert!(
        result.scores.keyword > 0.0,
        "Keyword score should be positive"
    );
}
...
fn test_large_number_of_routes() {
    ...
    // Build index
    let embeddings: Vec<Vec<f32>> = (0..50).map(|i| vec![(i as f32) / 50.0; 384]).collect();
